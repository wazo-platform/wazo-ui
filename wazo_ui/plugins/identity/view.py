...
class IdentityView(BaseIPBXHelperView):
    ...
    @menu_item(
        '.ipbx.identity', l_('Credentials'), icon="user-secret", multi_tenant=True
    )
    @menu_item(
...
class GroupView(BaseIPBXHelperView):
    ...
    @menu_item(
        '.ipbx.identity.groups', l_('Groups'), order=2, icon="users", multi_tenant=True
    )
    def index(self):
        return super().index()
...
class LDAPConfigView(BaseIPBXHelperView):
    ...
    @menu_item(
        '.ipbx.identity.ldap', l_('LDAP'), order=5, icon="wrench", multi_tenant=True
    )
