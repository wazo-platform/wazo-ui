...
class DirdProfileView(BaseIPBXHelperView):
    ...
    def _get(self, id, form=None):
        ...
        for service in form.services:
            service.uuid.choices = [(source['uuid'], source['name']) for source in sources['items']]
    ...
    def _map_resources_to_form(self, resource):
        ...
        sources = services['favorites']['sources'] + services['reverse']['sources'] + services['lookup']['sources']

        # Initialize all services to False for each sources
        for source in sources:
            if not source['uuid'] in services_by_uuid:
                services_by_uuid[source['uuid']] = {service_name: False for service_name in service_names}
