...
from nestbox_ui.helpers.classful import (
    LoginRequiredView,
...
class AccessFeatureView(BaseIPBXHelperView):
    ...
    @menu_item('.ipbx.advanced.access_features', l_('Features access'), icon="lock", multi_tenant=False)
    def index(self):
        return super().index()
...
class AccessFeaturesListingView(LoginRequiredView):
    ...
    def _list_json(self, type_=None, numeric_id=False):
        ...
        for access_feature in access_features['items']:
            access_feature_id = access_feature['id'] if numeric_id else access_feature['name']
            results.append({'id': access_feature_id, 'text': access_feature['label']})

        return jsonify(build_select2_response(results, access_features['total'], params))
